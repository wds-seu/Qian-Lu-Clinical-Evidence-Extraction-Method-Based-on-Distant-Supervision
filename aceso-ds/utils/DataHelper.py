...
class DataHelper(object):
    ...
    def load_sentences_cuis_and_labels(self, root, use_shuffle=False, use_drop=False):
        ...
        sentences_origin = []
        labels = []
        if os.path.exists(opt.word2cui):
            ...
            with open(os.path.join(root, file), encoding='ISO-8859-1') as f:
                ...
                for row in csv_reader:
                    ...
                    for word, cui in zip(tokens, cuis1):
                        if word not in word2cui:
                            word2cui[word] = cui
                    # todo: x和y添加了三遍，origin sentence只添加一遍
                    if use_shuffle:
                        np.random.seed(1)
    ...
    def load_vocab(self):
        if os.path.exists(opt.vocabulary_store):
            with open(opt.vocabulary_store, 'rb') as f:
                vocab_tuple = pickle.load(f)    # (vocab, vocab_inv)
                return vocab_tuple[0], vocab_tuple[1]
        else:
            vocabulary_store = {}
            vocabulary_store_inv = {}
            return vocabulary_store, vocabulary_store_inv
    ...
    def build_vocab(self, sentences=None):
        ...
        """
        vocabulary, vocabulary_inv = self.load_vocab()
        if self.test:
            return vocabulary, vocabulary_inv
        vocabulary_store_size = len(vocabulary)

        # vocabulary, vocabulary_inv = self.build_vocab(sentences_padded)
        word_counts = Counter(itertools.chain(*sentences))
        # Mapping from index to word
        word_dict = word_counts.most_common()
        words = [x[0] for x in word_dict]  # {index:str}
        words.append("UNKNOWN")
        # Mapping from word to index
        for word in words:
            if word not in vocabulary:
                vocabulary[word] = vocabulary_store_size
                vocabulary_inv[vocabulary_store_size] = word
                vocabulary_store_size += 1
    ...
    def load_data(self):
        ...
        cuis = [[int(cui[1:]) if (cui != "NULL" and cui != "") else 0 for cui in line] for line in cuis]
        vocabulary, vocabulary_inv = self.build_vocab(sentences_padded)
        if self.test:
            x_text = np.array(
                [[vocabulary[word] if word in vocabulary else vocabulary["UNKNOWN"] for word in sentence] for sentence
                 in sentences_padded])
            y = np.array(labels)
            y = y.reshape(-1, 1)
    ...
    def load_test_data(self, test_path):
        ...
        ans = {}
        vocabulary, vocabulary_inv = self.build_vocab()
        for file in files:
            sentences = []
            sentences2tokens = []
            sentences2cuis = []
            with open(os.path.join(test_path, file)) as f:
                ...
                for row in csv_reader:
                    ...
                    sentences2tokens.append(tokens)
                    cuis = row[2].split("|")
                    sentences2cuis.append(cuis)
            sentences_padded = self.pad_sentences_or_cuis(sentences2tokens, padding_str="<PAD/>", maxlen=28)
            cuis = self.pad_sentences_or_cuis(sentences2cuis, padding_str="G0000000", maxlen=28)
            cuis = [[int(cui[1:]) if (cui != "NULL" and cui != "") else 0 for cui in line] for line in cuis]
            x_text = np.array(
                [[vocabulary[word] if word in vocabulary else vocabulary["UNKNOWN"]
                  for word in sentence] for sentence in sentences_padded])
            ans[file] = [x_text, cuis, sentences]
        return ans
